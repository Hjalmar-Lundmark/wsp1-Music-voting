...
router.post('/deleteUser', async function (req, res, next) {
    ...
    const { votedSong } = req.body;

    // remove their vote
    if (votedSong.length > 3) {
        const [rows] = await promisePool.query("SELECT hl21music.votes FROM hl21music WHERE songId=?", votedSong);
        let count = rows[0].votes / 1; // javascript moment / convert to int
        count = count - 1;
        const [row] = await promisePool.query("UPDATE hl21music SET votes=? WHERE songId=?", [count, votedSong]);

        const [votedOn] = await promisePool.query("UPDATE hl21users2 SET votedOn=? WHERE id=?", [null, rowId]);
        // ^^ should be done
    }

    //delete from db
    await promisePool.query('DELETE FROM hl21users2 WHERE id=?', rowId);
    // ^^ should be done
